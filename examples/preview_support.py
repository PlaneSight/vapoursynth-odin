...
"""Shared synthetic inputs for interactive previews and documentation rendering."""
...
def load_plugin(example: str, namespace: str) -> None:
    ...
    path = ROOT / ".build" / "examples" / f"{example}{suffix}"
    if not path.is_file():
        raise RuntimeError(
            f"Missing example plugin: {path}\nRun: uv run tools/examples.py build {example}"
        )
    ...
    for plugin in vs.core.plugins():
        ...
        raise RuntimeError(
            ...
            "Use the repository uv environment without an installed example wheel, "
