...
The build command compiles the eight Odin binding examples, executes every plugin
...
Identity, invert, dither, and Hald CLUT each have a checked-in `preview.vpy` used by
...
that was compiled for that build. Each selected output exports frame zero.
