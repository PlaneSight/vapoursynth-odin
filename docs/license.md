...
styles, and example filter images are stored locally in `docs/assets`. The dither
and Hald CLUT comparison images are generated by running the repository's
examples. The inputs are generated locally, and `tools/render_showcase.py`
renders the filter output for the site.
